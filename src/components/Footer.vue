<template>
    <footer class="mt-[112px] bg-[#F5F5F6] py-[30px]">
        <div class="container mx-auto px-5 md:flex md:gap-[50px]">
            <div class="md:w-[33%]">
                <div class="w-[85%] flex items-center gap-2 md:w-[40%]">
                    <svg xmlns="http://www.w3.org/2000/svg" width="31" height="29" viewBox="0 0 31 29" fill="none">
                        <path d="M21.9011 0.517334C26.4897 0.517334 30.2084 4.29643 30.2084 9.58717C30.2084 20.1687 18.8802 26.2152 15.1042 28.4827C12.1166 26.6884 4.40438 22.5314 1.31104 15.6337H8.40699L9.81772 13.2816L14.349 20.8398L17.4695 15.6337H22.6563V12.6105H15.7597L14.349 14.9626L9.81772 7.40437L6.6972 12.6105H0.317188C0.111771 11.6475 0 10.6393 0 9.58717C0 4.29643 3.77605 0.517334 8.3073 0.517334C11.1167 0.517334 13.5938 2.02897 15.1042 3.54061C16.6146 2.02897 19.0917 0.517334 21.9011 0.517334Z" fill="#067A9E"/>
                    </svg>
                    <h1 class="font-bold text-[26.5px]">Shifo<span class="text-[#505069]">24</span></h1>
                </div>
                <p class="text-[16px] text-[#424259] font-[400] mt-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore </p>
            </div>
            <div class="mt-[40px] md:w-[30%] md:mt-0 xl:ml-[180px]">
                <h2 class="text-[16px] font-[700] text-[#424259]">Xizmatlar</h2>
                <div class="grid gap-[16px] mt-[16px]">
                    <router-link to="#" class="text-[16px] font-[400] text-[#424259]">Biz haqimizda</router-link>
                    <router-link to="#" class="text-[16px] font-[400] text-[#424259]">Maxfiylik va xavfsizlik</router-link>
                    <router-link to="#" class="text-[16px] font-[400] text-[#424259]">Qo’llab quvvatlash</router-link>
                </div>
            </div>
            <div class="mt-[40px] md:w-[40%] md:mt-0">
                <h2 class="text-[16px] font-[700] text-[#424259]">Aloqa raqami</h2>
                <div class="grid gap-[16px] mt-[16px]">
                    <p class="text-[16px] font-[400] text-[#2588AF]">+998 (99) 999-90-99</p>
                </div>
                <div class="mt-[24px]">
                    <h2 class="text-[16px] font-[700] text-[#424259]">Mobil ilovamizni yuklab oling</h2>
                    <div class="flex justify-center items-center gap-6 mx-auto mt-[24px]">
                        <a href="#"><img src="../assets/image/store/Play market.png" alt="playmarket" class="w-[80%] mx-auto"></a>
                        <a href="#"><img src="../assets/image/store/App Store Button.png" alt="AppStore" class="w-[80%]"></a>
                    </div>
                </div>
            </div>
        </div>
    </footer>
</template>

<script>
export default {
    setup () {
        

        return {}
    }
}
</script>

<style lang="scss" scoped>

</style>